...
func setupAPIV1Routes(app *httpx.App) {
	...
	api.Router.GET("/info", func(ctx context.Context, hc httpx.HandlerContext) {
		hc.AbortJSON(200, map[string]string{
			"version": "v1",
			"status":  "running",
		})
	...
	api.Router.GET("/timestamp", func(ctx context.Context, hc httpx.HandlerContext) {
		hc.AbortJSON(200, map[string]string{
			"unix": fmt.Sprintf("%d", time.Now().Unix()),
		})
	})
...
func setupAdminRoutes(app *httpx.App) {
	...
	admin.Router.GET("/dashboard", func(ctx context.Context, hc httpx.HandlerContext) {
		hc.AbortJSON(200, map[string]string{
	...
	admin.Router.GET("/stats", func(ctx context.Context, hc httpx.HandlerContext) {
		hc.AbortJSON(200, map[string]interface{}{
			"users":    len(users),
